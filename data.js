...
Practically, vLLM exposes an OpenAI-compatible API server, so it's often a drop-in replacement for hitting a hosted API when self-hosting open-weight models, and it supports tensor parallelism for splitting a model too large for one GPU across several. The interview-relevant tradeoff: vLLM optimizes for throughput (tokens/sec across many concurrent requests) more than single-request latency — for a single user waiting on one response, a simpler server can look competitive, but vLLM pulls ahead fast as concurrent request volume grows.`,
    ...
    flashcards: [
      ...
      { q: "FlashAttention", a: "An optimized way to compute attention that avoids unnecessary memory reads/writes. Makes both training and inference significantly faster without changing results.", image: { src: "assets/images/flash-attention-memory-compute.svg", alt: "Standard attention vs FlashAttention: memory and compute traffic. Standard attention repeatedly loads and writes full Q, K, S, P, V matrices between HBM memory and compute. FlashAttention loads blocks once, fuses softmax and output computation using running statistics l and m, and writes the output only once, drastically reducing memory reads and writes. Click to view full size." } },
...
A well-known trap in using click data as a relevance signal is position bias — users click higher-ranked results more just because they're higher-ranked, independent of true relevance — which, left uncorrected, creates a feedback loop where the current ranking's biases get baked into the training data for the next model. Techniques like randomizing result order for a small traffic slice, or explicitly modeling position bias in the training objective, are standard mitigations.`,
  ...
  },
  {
    ...
    category: "genai",
    tier: "common",
    image: { src: "assets/images/top-p-top-k-temperature-sampling-pipeline-cheatsheet.png", alt: "Sampling pipeline: logits -> temperature -> softmax -> top-k -> top-p -> renormalize -> sample" },
...
top-p ↓	fewer, more probable candidates`,
    ...
    flashcards: [
      ...
      { q: "top-p = 0?", a: "Net effect: top-p=0 behaves like top-k=1 → deterministic, greedy decoding.", example: "Cumulative probability reaches the threshold the instant the single highest-probability token is included, so only that one token survives." },
