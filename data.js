...
const DSA_CATEGORIES = [
  {
    ...
    icon: "🔤",
    problems: [
      { id: "as-two-sum", name: "Two Sum", difficulty: "Easy", hint: "A hash map of value → index turns an O(n²) search into a single O(n) pass." },
  ...
  },
  {
    ...
    icon: "🗂️",
    problems: [
      { id: "hm-group-anagrams", name: "Group Anagrams", difficulty: "Medium", hint: "Use each word's sorted-letters (or letter-count tuple) as the hash map key." },
      { id: "hm-contains-duplicate", name: "Contains Duplicate", difficulty: "Easy", hint: "A set that grows as you scan turns this into one pass with O(1) lookups." },
      { id: "hm-top-k-frequent", name: "Top K Frequent Elements", difficulty: "Medium", hint: "Count frequencies with a map, then use bucket sort by frequency to avoid a full sort." },
      { id: "hm-subarray-sum-k", name: "Subarray Sum Equals K", difficulty: "Medium", hint: "Track running prefix sums in a map; a subarray sums to k when prefix[j] - k has been seen before." },
      { id: "hm-longest-consecutive", name: "Longest Consecutive Sequence", difficulty: "Medium", hint: "Put all values in a set, then only start counting a run from numbers whose predecessor is absent." },
      { id: "hm-valid-anagram", name: "Valid Anagram", difficulty: "Easy", hint: "Compare letter-count maps of both strings, or sort both and check equality." },
    ],
  },
  {
    id: "two-pointers",
    ...
    problems: [
      ...
      { id: "tp-container-water", name: "Container With Most Water", difficulty: "Medium", hint: "Start pointers at both ends and always move the pointer at the shorter line inward." },
  ...
  },
  {
    ...
    icon: "🔗",
    problems: [
      { id: "ll-reverse", name: "Reverse Linked List", difficulty: "Easy", hint: "Walk the list once, re-pointing each node's next to the previous node as you go." },
      { id: "ll-merge-two-sorted", name: "Merge Two Sorted Lists", difficulty: "Easy", hint: "Use a dummy head node and repeatedly attach whichever list's current node is smaller." },
      { id: "ll-cycle", name: "Linked List Cycle", difficulty: "Easy", hint: "Floyd's fast/slow pointers meet inside a cycle if and only if one exists." },
      { id: "ll-remove-nth-end", name: "Remove Nth Node From End of List", difficulty: "Medium", hint: "Advance a lead pointer n steps first, then move both pointers together to find the target." },
      { id: "ll-reorder-list", name: "Reorder List", difficulty: "Medium", hint: "Find the middle, reverse the second half, then merge the two halves alternately." },
      { id: "ll-copy-random-pointer", name: "Copy List with Random Pointer", difficulty: "Medium", hint: "A hash map from original node to its clone lets you wire up random pointers in a second pass." },
    ],
  },
  {
    id: "trees",
    ...
    icon: "🌳",
    problems: [
      { id: "tr-max-depth", name: "Maximum Depth of Binary Tree", difficulty: "Easy", hint: "1 + the larger of the max depths of the left and right subtrees, recursively." },
      { id: "tr-validate-bst", name: "Validate Binary Search Tree", difficulty: "Medium", hint: "Recurse with a valid (low, high) range per node rather than only comparing to immediate children." },
      { id: "tr-level-order", name: "Binary Tree Level Order Traversal", difficulty: "Medium", hint: "A queue-based BFS, processing one full level's worth of nodes before moving to the next." },
      { id: "tr-lca-bst", name: "Lowest Common Ancestor of a BST", difficulty: "Medium", hint: "Use BST ordering: branch left if both targets are smaller, right if both are larger, else you're at the LCA." },
      { id: "tr-invert-tree", name: "Invert Binary Tree", difficulty: "Easy", hint: "At each node, swap the left and right children, then recurse into both." },
      { id: "tr-serialize-deserialize", name: "Serialize and Deserialize Binary Tree", difficulty: "Hard", hint: "Preorder traversal with explicit null markers lets you reconstruct the tree unambiguously." },
      { id: "tr-diameter", name: "Diameter of Binary Tree", difficulty: "Easy", hint: "At every node, track left-depth + right-depth as a candidate diameter while computing height." },
    ],
  },
  {
    id: "graphs",
    ...
    icon: "🕸️",
    problems: [
      { id: "gr-number-islands", name: "Number of Islands", difficulty: "Medium", hint: "DFS or BFS flood-fill from each unvisited land cell, marking every connected cell visited." },
      { id: "gr-course-schedule", name: "Course Schedule", difficulty: "Medium", hint: "This is cycle detection on a directed graph — topological sort succeeds only if no cycle exists." },
      { id: "gr-clone-graph", name: "Clone Graph", difficulty: "Medium", hint: "DFS/BFS with a map from original node to its clone avoids infinite loops on cycles." },
      { id: "gr-word-ladder", name: "Word Ladder", difficulty: "Hard", hint: "BFS over words where an edge connects words differing by exactly one letter finds the shortest path." },
      { id: "gr-pacific-atlantic", name: "Pacific Atlantic Water Flow", difficulty: "Medium", hint: "Run BFS/DFS inward from each ocean's border cells and intersect the two reachable sets." },
      { id: "gr-valid-tree", name: "Graph Valid Tree", difficulty: "Medium", hint: "A graph is a valid tree exactly when it's connected and has exactly n-1 edges (no cycles)." },
    ],
  },
  {
    id: "heaps",
    ...
    icon: "⛰️",
    problems: [
      { id: "hp-kth-largest", name: "Kth Largest Element in an Array", difficulty: "Medium", hint: "Maintain a min-heap of size k; the heap's root is the answer once it's full." },
      { id: "hp-merge-k-sorted", name: "Merge K Sorted Lists", difficulty: "Hard", hint: "A min-heap holding one node per list lets you always pop the next-smallest across all lists." },
      { id: "hp-top-k-words", name: "Top K Frequent Words", difficulty: "Medium", hint: "Count frequencies, then use a heap with a comparator that breaks ties alphabetically." },
      { id: "hp-median-stream", name: "Find Median from Data Stream", difficulty: "Hard", hint: "Two heaps — a max-heap for the lower half, a min-heap for the upper half — kept balanced in size." },
      { id: "hp-task-scheduler", name: "Task Scheduler", difficulty: "Medium", hint: "Greedily schedule the most frequent remaining task first, using a max-heap plus a cooldown queue." },
    ],
  },
  {
    ...
    icon: "🧩",
    problems: [
      { id: "dp-climbing-stairs", name: "Climbing Stairs", difficulty: "Easy", hint: "ways(n) = ways(n-1) + ways(n-2) — it's just Fibonacci in disguise." },
      { id: "dp-coin-change", name: "Coin Change", difficulty: "Medium", hint: "Bottom-up: minCoins[amount] = 1 + min over each coin of minCoins[amount - coin]." },
      { id: "dp-lis", name: "Longest Increasing Subsequence", difficulty: "Medium", hint: "dp[i] = length of the longest increasing subsequence ending exactly at index i." },
      { id: "dp-lcs", name: "Longest Common Subsequence", difficulty: "Medium", hint: "2D table where dp[i][j] depends on whether characters i and j match, building from empty prefixes." },
      { id: "dp-house-robber", name: "House Robber", difficulty: "Medium", hint: "At each house, decide max(skip this house, rob it + best from two houses back)." },
      { id: "dp-word-break", name: "Word Break", difficulty: "Medium", hint: "dp[i] is true if some earlier dp[j] is true and the substring from j to i is a dictionary word." },
      { id: "dp-edit-distance", name: "Edit Distance", difficulty: "Hard", hint: "2D table over prefixes of both strings; each cell picks the cheapest of insert, delete, or replace." },
    ],
  },
  {
    id: "sql",
    ...
    icon: "🗄️",
    problems: [
      { id: "sql-second-highest-salary", name: "Second Highest Salary", difficulty: "Medium", hint: "DISTINCT plus ORDER BY/LIMIT with an OFFSET, or a subquery excluding the MAX, both work." },
      { id: "sql-employees-more-than-managers", name: "Employees Earning More Than Their Managers", difficulty: "Easy", hint: "Self-join the employee table to itself on the manager id column." },
      { id: "sql-duplicate-emails", name: "Duplicate Emails", difficulty: "Easy", hint: "GROUP BY the email column and filter groups with HAVING COUNT(*) > 1." },
      { id: "sql-rising-temperature", name: "Rising Temperature", difficulty: "Easy", hint: "Self-join the table on date = date - 1 day and compare each day's temperature to the prior day's." },
      { id: "sql-dept-top-three-salaries", name: "Department Top Three Salaries", difficulty: "Hard", hint: "A window function like DENSE_RANK partitioned by department handles ties correctly." },
      { id: "sql-consecutive-numbers", name: "Consecutive Numbers", difficulty: "Medium", hint: "Use LEAD/LAG window functions, or self-join the log table three times on consecutive ids." },
    ],
  },
  {
    id: "pandas",
